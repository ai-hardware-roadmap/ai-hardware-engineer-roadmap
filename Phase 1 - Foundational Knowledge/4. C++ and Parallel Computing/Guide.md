...
**What ROCm is:** AMD's open-source GPU compute platform (Radeon Open Compute). It includes the HIP programming language, kernel driver (`amdgpu`), runtime, compiler (`amd-clang`), math libraries (rocBLAS, MIOpen, rocFFT), and profiling tools (Omniperf, Omnitrace). ROCm is to AMD what CUDA is to NVIDIA.
...
#### CDNA vs RDNA — AMD's Two GPU Architectures
...
#### CUDA vs HIP — Almost Identical API
...
#### AMD GPU Architecture — CDNA Compute Unit Deep Dive
