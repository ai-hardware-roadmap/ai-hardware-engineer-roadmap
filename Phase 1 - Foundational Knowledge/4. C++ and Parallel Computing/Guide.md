...
**Full guide:** [**ROCm and HIP Guide →**](ROCm%20and%20HIP/Guide.md)

**What you'll learn:** AMD's two GPU architectures (CDNA for data center AI vs RDNA for gaming), HIP programming (95% identical to CUDA), HIPIFY toolchain for automatic CUDA→HIP conversion, AMD Compute Unit architecture, ROCm software stack and library ecosystem.

**Key topics in the sub-guide:**
1. **CDNA vs RDNA** — why MI300X (CDNA, compute-only, 192 GB HBM3) is completely different from Radeon RX (RDNA, gaming)
2. **HIP API** — CUDA-to-HIP mapping table, wavefront (64) vs warp (32) differences, code examples
3. **HIPIFY deep dive** — hipify-clang vs hipify-perl, what auto-converts, what needs manual work, full project porting workflow
4. **CDNA Compute Unit** — internal architecture diagram, SM vs CU comparison, optimization differences
5. **ROCm stack** — software layers, CUDA-X → ROCm library mapping (rocBLAS, MIOpen, RCCL, CK), profiling (Omniperf, Omnitrace)

**Projects:** HIPIFY a CUDA kernel, wavefront vs warp reduction, Omniperf roofline, rocBLAS benchmark, port a real CUDA project
