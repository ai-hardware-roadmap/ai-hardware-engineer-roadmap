...
- The TensorRT and CUDA packages in the Yocto image (`cuda-toolkit`, `tensorrt`) are the same userspace libraries studied in Phase 4, but now you control which version is on the device — critical for ensuring model compatibility across a fleet of devices that may have been manufactured months apart.
